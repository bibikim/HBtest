package ex01branch;

public class Ex04_switch {

	public static void main(String[] args) {
		// switch문
		// 조건을 쓰지 않고 표현식의 결과값에 따른 분기를 처리한다
		// 딱 떨어지는 값, 정확한 값에 의해 동작한다. 범위 없음
		// 표현식의 결과값은 double, boolean 데이터타입일 수 없다.
		// switch(표현식) {
		// case 값: 실행문; break;
		// case 값: 실행문; break;
		// default: 실행문;                    default 생략가능
		// }
		
		int step = 2;
		
		switch(step) {
		case 1: System.out.println("1단계"); break;
		case 2: System.out.println("2단계"); break;
		case 3: System.out.println("3단계"); break;
		default: System.out.println("땡");
		}
		
		// 연습.
		// 각 층별 관리자를 출력
		// 1 ~ 2층 : 전지현
		// 3 ~ 4층 : 한지민
		// 5 ~ 6층 : 박은빈
		// 나머지층: 박보검		
			int floor = 5;
			String manager;
		switch(floor) {
		case 1:
		case 2:
			manager = "전지현"; break;
		case 3:
		case 4:
			manager = "한지민"; break;
		case 5:
		case 6:
			manager = "박은빈"; break;
		default:
			manager = "박보검";
		}
		System.out.println(floor + "층 관리자는 " + manager + "입니다");
		
		
		// 연습.
		// 짝수, 홀수
		int n = 8;
		switch(n % 2) {
		case 1: System.out.println("홀수"); break;
		default: System.out.println("짝수");
		}
		
		// 연습.
		// 1~4분기 출력하기 
		int month = 13;
		switch((month - 1) / 3) {
		case 0: System.out.println("1분기"); break;
		case 1: System.out.println("2분기"); break;
		case 2: System.out.println("3분기"); break;
		case 3: System.out.println("4분기"); break;
		}

		// 연습.
		// 점수에 따른 학점
		int score = 93;
		String grade;
		switch(score / 10) {
		case 10:
		case 9:
			grade = "A"; break;
		case 8:
			grade = "B"; break;
		case 7:
			grade = "C"; break;
		case 6:
			grade = "D"; break;
		default:
			grade = "F";
		}
		System.out.println(score +"점은 " + grade + "입니다.");
		
		// 연습.
		// 등급(1,2,3)에 따른 권한 출력
		// 1등급 : 쓰기 실행 읽기
		// 2등급 : 실행 읽기
		// 3등급 : 읽기
		// 나머지 : 없음
		int lev = 2;
		String kh = "";        //문자열들을 누적시키는 경우는 비워두고 += 복합연산 사용!
		switch(lev) {
		case 1: kh += "쓰기 ";
		case 2: kh += "실행 ";
		case 3: kh += "읽기"; break;
		default: kh += "없음";
		}
		
		System.out.println(lev + "등급: " + kh);
		
		
		
		
		
		
	}

}
